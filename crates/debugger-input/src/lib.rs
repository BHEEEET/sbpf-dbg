...
pub fn serialize_parameters(
    ...
    for account in accounts {
        match account {
            SerializeAccount::Acc(_, account) => {
...
pub fn generate(instruction: &Instruction, output_name: &str) -> Result<(), DebuggerInputError> {
    ...
    for (i, account_meta) in instruction.accounts.iter().enumerate() {
        if let Some(&first_index) = seen_pubkeys.get(&account_meta.pubkey) {
            ...
            accounts.push(SerializeAccount::Duplicate(first_index as u8));
        } else {
            ...
            accounts.push(SerializeAccount::Acc(i, account));
        }
    }

    // Serialize the parameters.
    let serialized_data =
        serialize_parameters(accounts, &instruction.data, &instruction.program_id)?;
...
mod tests {
    ...
    fn test_serialize_parameters() {
        ...
        let result = generate(&instruction, "test_output.hex");
        assert!(result.is_ok());
    }
    ...
    fn test_serialize_parameters_with_duplicates() {
        ...
        let result = generate(&instruction, "test_duplicates.hex");
        assert!(result.is_ok());
    }
}
